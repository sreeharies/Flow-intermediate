...
pub contract jio: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        destroy() {
            jio.totalSupply = jio.totalSupply - self.balance
        }
    }
    pub resource Admin {
    ...
    pub resource Minter {
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            jio.totalSupply = jio.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&jio.Minter>(/public/Minter, target: /storage/MinterStorage)
